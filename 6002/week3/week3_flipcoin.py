...
def flipCoin(numFlips):
    ...
    '''
    with open('week3_coin_flips.txt','r') as f:
        all_flips = f.read()
...
def clt():
    for sampleSize in sampleSizes:
        sampleMeans = []
        for t in range(20):
            sample = flipCoin(sampleSize) #my edit
            sampleMeans.append(getMeanAndStd(sample)[0])
        meany, stdy = getMeanAndStd(sampleMeans)
        meanOfMeans.append(meany) #my edit
        stdOfMeans.append(stdy) #my edit
