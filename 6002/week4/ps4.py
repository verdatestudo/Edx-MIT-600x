...
def r_squared(y, estimated):
    ...
    mean = sum(y) / len(y)
    top = sum(((y[idx] - estimated[idx]) ** 2 for idx in range(len(y))))
    bottom = sum(((y[idx] - mean) ** 2 for idx in range(len(y))))
    return 1 - float(top/bottom)
...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        ...
        points = plt.plot(x, y, 'bo', label = 'Measured points')
        best_fit = plt.plot(x, est_y, 'r', label = 'Model')

        plt.title('Temperature in Boston on January 10th on various years \n\
        Regression model degree = %d, R-Squared = %f' %((len(model) - 1), r_squared(y, est_y)))
        plt.xlabel('Year)')
